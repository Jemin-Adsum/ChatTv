...
class FileReader(private val activity: Activity, private val fileName: Uri,val interstitialAds: InterstitialAds) {
    ...
    fun readFile() {
        try {
            val inputStreamReader = activity.contentResolver.openInputStream(
                fileName
            )
            val bufferedReader: BufferedReader? = BufferedReader(InputStreamReader(inputStreamReader))
            ...
            while (bufferedReader?.readLine().also {
                ...
                try {
                    ...
                    if (currentLine?.startsWith(EXT_INF_SP) == true) {
                        channelName =
